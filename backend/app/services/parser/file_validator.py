...
def validate_filename(filename: str) -> str:
    ...
    if "/" in filename or "\\" in filename or ".." in filename:
        raise ValidationError("Filename contains path traversal characters", details="PATH_TRAVERSAL_ATTEMPT")

    # Whitelist characters (alphanumeric, spaces, dots, dashes, underscores)
    if not re.match(r'^[\w\-. ]+$', filename):
        raise ValidationError("Filename contains invalid or suspicious characters", details="SUSPICIOUS_FILENAME")
...
def validate_file_content(file_bytes: bytes, filename: str) -> None:
    ...
    if ext == ".pdf":
        if not file_bytes.startswith(b"%PDF"):
            raise ValidationError("File signature mismatch: Not a valid PDF file.", details="INVALID_PDF_SIGNATURE")
    elif ext == ".docx":
        if not file_bytes.startswith(b"PK\x03\x04"):
            raise ValidationError("File signature mismatch: Not a valid DOCX file.", details="INVALID_DOCX_SIGNATURE")
