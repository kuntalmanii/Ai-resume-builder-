...
class LLMClaimVerification(BaseModel):
    ...
    verification_status: str = Field(
        ..., description="source_verified, user_confirmed, inferred, unverified, contradictory"
    )
    contradiction_details: str | None = Field(
        None, description="Details if the claim contradicts known facts"
    )
...
class CredibilityEngineService:
    @classmethod
    async def compute_evidence_state_fingerprint(
        cls, db: AsyncSession, resume_id: uuid.UUID, user_id: uuid.UUID
    ) -> str:
        """Generate a fingerprint of the current state of career profile data, claims,
        and confirmations."""
        # 1. Fetch CareerEntries
        entries_stmt = (
            select(CareerEntry).where(CareerEntry.user_id == user_id).order_by(CareerEntry.id)
        )
        entries = list(await db.scalars(entries_stmt))

        # 2. Fetch ResumeClaims
        claims_stmt = (
            select(ResumeClaim).where(ResumeClaim.resume_id == resume_id).order_by(ResumeClaim.id)
        )
        ...
        for ent in entries:
            state_parts.append(
                f"entry:{ent.id}:{ent.updated_at.isoformat()}:{ent.verification_status}"
            )
        for c in claims:
    ...
    def check_contradictions(cls, claim: ResumeClaim, facts: list[dict[str, Any]]) -> str | None:
        ...
        if (
            ...
            and claim.field_name == "is_current"
            and claim.normalized_value == "current"
        ):
            for f in facts:
    ...
    def is_similar_achievement(cls, txt1: str, txt2: str) -> bool:
        """Heuristic check to see if two bullet lines describe the same
    ...
    @classmethod
    async def _fetch_user_facts_list(
            ...
            facts.append(
                {
                    ...
                    "bullets": ec.get("bullets") or [ec.get("description")]
                    if ec.get("bullets") or ec.get("description")
                    else [],
    ...
    async def verify_claim_deterministically(
        cls, claim: ResumeClaim, facts: list[dict[str, Any]]
        ...
        evidence = []
        cls.normalize_text(claim.claim_text)
        ...
        # Check for self-reported (e.g. responsibility claims or qualitative statements
        # that don't need external verification)
    ...
    async def compute_evidence_map(
        ...
        [
            c
            for c in claims
            if c.verification_status in ["unverified", "unsupported", "career_profile_supported"]
        ]
        ...
        for claim in claims:
            ...
            status, contra_details, evidence_list = await cls.verify_claim_deterministically(
                claim, facts
            )
        ...
        cnt_supported = sum(
            ...
            in ["source_verified", "user_confirmed", "career_profile_supported"]
        )
        cnt_contradictions = sum(1 for c in claims if c.verification_status == "contradictory")
        cnt_career_profile = sum(
            1 for c in claims if c.verification_status == "career_profile_supported"
        )
        ...
        cnt_verified_evidence = sum(
            sum(
                1
                for ev in c.evidence_sources
                if ev.verification_status in ["source_verified", "user_confirmed"]
            )
            for c in claims
        )
        ...
        applicable_max = (
            40.0 + 20.0 + 15.0
        )  # Claim Support, Consistency, and Profile Corroboration are always applicable
