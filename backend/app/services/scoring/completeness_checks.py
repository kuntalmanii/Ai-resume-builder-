...
def check_personal_info(resume: Any) -> CheckResult:
    ...
    if filled == 3:
        return CheckResult(
            code, CATEGORY, "Personal Information Complete",
        ...
        return CheckResult(
            code, CATEGORY, "Personal Information Mostly Complete",
        ...
        return CheckResult(
            code, CATEGORY, "Personal Information Sparse",
            "Multiple personal information fields are missing.",
            "failed", possible, 0,
            recommendation="Add your name, email address, and location to the personal information section.",
        )
...
def check_foundation(resume: Any) -> CheckResult:
    ...
    if has_exp and has_edu:
        return CheckResult(
            code, CATEGORY, "Strong Foundation — Experience & Education",
        ...
        return CheckResult(
            code, CATEGORY, f"Foundation Present — {foundation_type.title()}",
            f"Resume has {foundation_type}. Adding the other would strengthen the profile.",
            "warning", possible, possible - 1,
            recommendation=f"Consider adding {'education' if has_exp else 'work experience or internships'} to build a stronger foundation.",
        )
    else:
        return CheckResult(
            code, CATEGORY, "No Foundation — Missing Experience and Education",
            "Neither work experience nor education entries are present.",
            "failed", possible, 0,
            recommendation="Add your educational background and/or any work experience, internships, or projects.",
        )
...
def check_skills(resume: Any) -> CheckResult:
    ...
    if total_skills >= 5:
        return CheckResult(
            code, CATEGORY, "Skills Section Complete",
        ...
        return CheckResult(
            code, CATEGORY, "Skills Section Sparse",
            f"Only {total_skills} skill(s) listed. A more complete skills section helps ATS matching.",
            "warning", possible, possible - 1,
            recommendation="Expand your skills section with technical skills, tools, and soft skills.",
            evidence_data={"skill_count": total_skills},
        )
    else:
        return CheckResult(
            code, CATEGORY, "Skills Section Missing",
            "No skills listed. This significantly reduces ATS keyword matching potential.",
            "failed", possible, 0,
            recommendation="Add a Skills section with your technical skills, tools, and relevant competencies.",
        )
...
def check_evidence(resume: Any) -> CheckResult:
    ...
    if total_evidence >= 3:
        return CheckResult(
            code, CATEGORY, "Strong Evidence of Capability",
            f"Resume demonstrates capability through {len(experience)} experience and {len(projects)} project entries.",
            "passed", possible, possible,
            evidence_data={"experience_count": len(experience), "project_count": len(projects)},
        )
    elif total_evidence >= 1:
        return CheckResult(
            code, CATEGORY, "Some Evidence Present",
            f"Resume shows {total_evidence} evidence entries. Adding more strengthens the profile.",
            "warning", possible, possible - 1,
            recommendation="Add more experience entries, internships, or personal projects to demonstrate capability.",
            evidence_data={"experience_count": len(experience), "project_count": len(projects)},
        )
    else:
        return CheckResult(
            code, CATEGORY, "No Evidence of Capability",
            "No experience or project entries found.",
            "failed", possible, 0,
            recommendation="Add work experience, internships, or personal/academic projects to showcase your capabilities.",
        )
...
def check_summary(resume: Any) -> CheckResult:
    ...
    if len(summary.split()) >= 20:
        return CheckResult(
            code, CATEGORY, "Professional Summary Present",
            "Resume includes a professional summary to contextualize the application.",
            "passed", possible, possible,
        )
    elif summary:
        return CheckResult(
            code, CATEGORY, "Summary Too Brief",
            "Professional summary exists but is very short (under 20 words).",
            "warning", possible, 0,
            recommendation="Expand the summary to 2–4 sentences covering your role, skills, and key value proposition.",
        )
    else:
        return CheckResult(
            code, CATEGORY, "No Professional Summary",
            "No professional summary provided. A summary helps frame the resume for recruiters.",
            "warning", possible, 0,
            recommendation="Add a 2–4 sentence professional summary that highlights your background and value.",
        )


def check_credibility_sections(resume: Any) -> CheckResult:
    """COMPLETE_CREDIBILITY — Optional sections that add credibility (certifications, achievements, etc.)."""
    ...
    if present_sections:
        return CheckResult(
            code, CATEGORY, "Credibility Sections Present",
        ...
        return CheckResult(
            code, CATEGORY, "No Additional Credibility Sections",
            "No certifications, achievements, languages, or leadership positions listed.",
            "warning", possible, 0,
            recommendation="Consider adding certifications, achievements, or language proficiencies to strengthen credibility.",
        )
