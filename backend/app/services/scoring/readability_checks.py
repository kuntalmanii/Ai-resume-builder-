...
def check_bullet_length(resume: Any) -> CheckResult:
    ...
    if not bullets:
        return CheckResult(
            code, CATEGORY, "Bullet Length — No Bullets",
    ...
    if not long_bullets:
        return CheckResult(
            code, CATEGORY, "Bullet Lengths Appropriate",
        ...
        return CheckResult(
            ...
            recommendation=f"Shorten {len(long_bullets)} long bullet(s). Aim for 12–25 words per bullet.",
            evidence_data={"long_bullet_count": len(long_bullets)},
        )
    else:
        return CheckResult(
            code, CATEGORY, "Many Bullets Exceed Readable Length",
            f"{len(long_bullets)}/{len(bullets)} bullets exceed {MAX_BULLET_WORDS} words — hard to scan quickly.",
            "failed", possible, 0,
            recommendation="Trim long bullets to their key contribution. Remove unnecessary context from individual bullets.",
...
def check_sentence_complexity(resume: Any) -> CheckResult:
    ...
    if not bullets:
        return CheckResult(
            code, CATEGORY, "Sentence Complexity — No Bullets",
            "No bullet points to evaluate.",
            "passed", possible, possible,
        )
    ...
    if not multi_sentence:
        ...
        return CheckResult(
            code, CATEGORY, "Many Bullets Are Multi-Sentence",
            f"{len(multi_sentence)} bullets read as paragraphs rather than crisp statements.",
            "failed", possible, 0,
            recommendation="Each bullet should make a single point. Split multi-sentence bullets into separate items.",
            evidence_data={"multi_sentence_count": len(multi_sentence)},
        )
...
def check_excessive_paragraphs(resume: Any) -> CheckResult:
    ...
    if summary_words <= EXCESSIVE_SUMMARY_WORDS and not long_descriptions:
        return CheckResult(
            code, CATEGORY, "No Excessive Paragraphs",
        ...
        return CheckResult(
            code, CATEGORY, "Summary Is Too Long",
            f"Professional summary is {summary_words} words — exceeds recommended length of {EXCESSIVE_SUMMARY_WORDS} words.",
            "warning", possible, possible - 1,
            recommendation="Trim the professional summary to 3–5 sentences (50–80 words). Move details to the experience section.",
            evidence_data={"summary_word_count": summary_words},
        )
    else:
        return CheckResult(
            code, CATEGORY, "Experience Entries Lack Bullets",
            f"{len(long_descriptions)} experience entries use descriptions instead of bullet points.",
            "warning", possible, possible - 1,
            recommendation="Convert experience descriptions to bullet points for better readability and ATS parsing.",
...
def check_repetition(resume: Any) -> CheckResult:
    ...
    if not duplicates:
        ...
        return CheckResult(
            code, CATEGORY, "Significant Repetition",
            f"{len(duplicates)} duplicate bullet pairs make the resume feel repetitive.",
            "failed", possible, 0,
            recommendation="Ensure each bullet highlights a unique contribution. Remove copy-pasted bullets.",
            evidence_data={"duplicate_pairs": len(duplicates)},
        )
...
def check_density_balance(resume: Any) -> CheckResult:
    ...
    total_sections_populated = sum([
        bool(experience_count), bool(education_count),
        bool(projects_count), bool(skills_count),
    ])
    ...
    if total_sections_populated >= 3 and (experience_count == 0 or avg_bullets_per_exp >= 2):
        return CheckResult(
            code, CATEGORY, "Good Section Balance",
        ...
        return CheckResult(
            code, CATEGORY, "Moderate Section Balance",
            "Most sections have content, but some are sparse.",
            "warning", possible, possible - 1,
            recommendation="Add 2–4 bullet points per experience entry and fill in sparse sections.",
            evidence_data={"sections_populated": total_sections_populated},
        )
    else:
        return CheckResult(
            code, CATEGORY, "Unbalanced Resume Structure",
            "Only a few sections have content — the resume appears one-dimensional.",
            "failed", possible, 0,
            recommendation="Populate multiple sections (experience, education, skills, projects) for a well-rounded resume.",
            evidence_data={"sections_populated": total_sections_populated},
        )
