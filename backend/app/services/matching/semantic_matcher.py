    ...
    if has_ai:
        try:
            ...
            prompt = (
                ...
                "Example: 'Build and maintain RESTful APIs' matches 'Developed Node.js backend endpoints'.\n"
                "Rules:\n"
                "1. Only match items with high confidence (>= 0.75). If related but not equivalent, set confidence low.\n"
                "2. Provide a brief (1-sentence) explanation justifying the match.\n"
                "3. Do not map a missing required hard skill (like Kafka) to a generic skill (like backend development).\n\n"
            ...
            for item in result.matches:
                if item.confidence >= 0.75:
                    semantic_matches.append({
                        ...
                        "requirement_text": next((r.text for r in unmatched_reqs if r.id == item.requirement_id), ""),
