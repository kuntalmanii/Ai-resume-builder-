...
@router.post(
    "/{id}/suggestions", response_model=SuggestionResponse, status_code=status.HTTP_201_CREATED
)
async def generate_suggestion(
    id: uuid.UUID, req: SuggestionGenerateRequest, db: DBSession, current_user: CurrentUser
) -> SuggestionResponse:
...
@router.post(
    "/{id}/suggestions/batch",
    response_model=list[SuggestionResponse],
    status_code=status.HTTP_201_CREATED,
)
async def batch_generate_suggestions(
    id: uuid.UUID, req: SuggestionBatchGenerateRequest, db: DBSession, current_user: CurrentUser
) -> list[SuggestionResponse]:
...
async def get_suggestion(
    id: uuid.UUID, suggestion_id: uuid.UUID, db: DBSession, current_user: CurrentUser
) -> SuggestionResponse:
...
async def edit_suggestion(
    ...
    sugg = await AISuggestionService.edit_suggestion(
        db, suggestion_id, req.suggested_text, current_user.id
    )
    return SuggestionResponse.model_validate(sugg)
...
async def answer_clarifying_question(
    ...
    sugg = await AISuggestionService.answer_clarifying_question(
        db, suggestion_id, req.answer, current_user.id
    )
    return SuggestionResponse.model_validate(sugg)
