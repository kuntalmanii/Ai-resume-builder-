...
@router.post("/resumes/{resume_id}/exports", response_model=ResumeExportResponse, status_code=status.HTTP_201_CREATED)
...
async def list_resume_exports(
    ...
    # Get exports
    export_stmt = select(ResumeExport).where(ResumeExport.resume_id == resume_id).order_by(desc(ResumeExport.created_at))
...
async def get_export_details(
    ...
    stmt = select(ResumeExport).options(selectinload(ResumeExport.resume)).where(ResumeExport.id == export_id)
    res = await db.execute(stmt)
...
async def download_export_pdf(
    ...
    if not export_snapshot.storage_path or not export_snapshot.storage_path.startswith(os.path.abspath("uploads/exports")):
        raise ForbiddenError("Invalid file path path traversal detected")

    filename = f"Resume_{export_snapshot.resume.title.replace(' ', '_')}_v{export_snapshot.resume_version}.pdf"

    return FileResponse(
        path=export_snapshot.storage_path,
