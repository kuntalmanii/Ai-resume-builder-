...
class JobDescriptionRequirement(BaseModel):
    ...
    extraction_method: str = Field(
        default="deterministic", description="deterministic | llm_structured | hybrid"
    )
...
class JobDescriptionRequirements(BaseModel):
    ...
    seniority: str | None = Field(
        None, description="Extracted seniority level (e.g. Senior, Junior, Lead)"
    )
    required_skills: list[str] = Field(
        default_factory=list, description="List of required skill keywords"
    )
    preferred_skills: list[str] = Field(
        default_factory=list, description="List of preferred skill keywords"
    )
    responsibilities: list[str] = Field(
        default_factory=list, description="List of responsibilities or tasks"
    )
    required_experience: str | None = Field(
        None, description="Extracted required years of experience description"
    )
    preferred_experience: str | None = Field(
        None, description="Extracted preferred years of experience description"
    )
    education_requirements: list[str] = Field(
        default_factory=list, description="Education requirements (e.g. BS in CS)"
    )
    certification_requirements: list[str] = Field(
        default_factory=list, description="Certification requirements (e.g. AWS Developer)"
    )
    domain_keywords: list[str] = Field(
        default_factory=list, description="General industry domain keywords"
    )
    tools_and_technologies: list[str] = Field(
        default_factory=list, description="Tools and technologies requested"
    )
    soft_skills: list[str] = Field(default_factory=list, description="Soft skills requested")
    raw_requirement_fragments: list[JobDescriptionRequirement] = Field(
        default_factory=list, description="Flat list of all parsed requirement nodes"
    )
